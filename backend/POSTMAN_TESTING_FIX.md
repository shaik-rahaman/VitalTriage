...
- **Problem:** Using port `3000` instead of `8000`
- **Solution:** Changed all requests from `http://localhost:3000/...` to `http://localhost:8000/...`
- **Why:** Backend runs on port 8000 (Uvicorn default), not 3000
...
1. **Use Postman with correct port:** `http://localhost:8000`
2. **Import the collection:** `VitalTriage_API.postman_collection.json`
3. **Select environment:** `VitalTriage Environment` with `baseUrl: http://localhost:8000`
...
### Why Port 8000?
Uvicorn (ASGI server) binds to `127.0.0.1:8000` by default
```bash
...
5. All requests use `{{baseUrl}}` which resolves to `http://localhost:8000`
